...
def parsing_news_BBC():
    ...
    --------------------from app-----------------------
        engine = create_engine('sqlite:///instance/blog.db')  # Пример подключения к SQLite базе данных

        # Проверка уникальности данных перед добавлением в базу данных
        session = sessionmaker(bind=engine)()

        if not session.query(Article).filter_by(title=article.title).first() and response != "https://error":
            try:
                session.add(article)
                session.commit()
